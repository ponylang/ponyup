...
class DLHandler is HTTPHandler
  ...
  new create(dl_dump: DLDump) =>
    _dl_dump = dl_dump

  fun apply(res: Payload val) =>
    _dl_dump(res)

  fun chunk(bs: ByteSeq val) =>
    _dl_dump.chunk(bs)
...
actor DLDump
  ...
  new create(out: OutStream, file_path: FilePath) =>
    _out = out
  ...
  be chunk(bs: ByteSeq val) =>
    ...
    for i in Range(0, 100, 5) do progress_bar.append(
  ...
  be finished() =>
    _out.write("\n")
    // TOOD: callback to ponyup, decompress into dir
